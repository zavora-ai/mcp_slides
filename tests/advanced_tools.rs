...
fn set_footer_tool() {
    ...
    let r = s.call(
        id,
        "set_footer",
        json!({"handle": &h, "slide": 0, "text": "Confidential", "visible": true}),
    );
    ...
    assert!(
        r["status"] == "success" || r["category"] == "invalid_input",
...
fn manifest_tool_parity() {
    ...
    assert!(
        tool_count >= 71,
        "Expected at least 71 tools, got {tool_count}"
    );
}
