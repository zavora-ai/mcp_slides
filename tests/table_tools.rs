...
fn open_deck_with_table(s: &mut Server, id: &mut i64, suffix: &str) -> (String, std::path::PathBuf) {
    ...
    *id += 1;
    ...
    // Save + reopen
    let path = std::env::temp_dir().join(format!("slides_mcp_tbl_{suffix}_{}.pptx", std::process::id()));
...
fn table_add_remove_row() {
    ...
    let r = s.call(id, "table_add_row", json!({"handle": &h, "slide": 0, "shape_idx": 0, "height_in": 0.5}));
    id += 1;
    assert_eq!(r["status"], "success", "table_add_row failed: {r}");

    let r = s.call(id, "table_remove_row", json!({"handle": &h, "slide": 0, "shape_idx": 0, "row_idx": 0}));
    id += 1;
...
fn table_add_remove_column() {
    ...
    let r = s.call(id, "table_add_column", json!({"handle": &h, "slide": 0, "shape_idx": 0, "width_in": 1.5}));
    id += 1;
    assert_eq!(r["status"], "success", "table_add_column failed: {r}");

    let r = s.call(id, "table_remove_column", json!({"handle": &h, "slide": 0, "shape_idx": 0, "col_idx": 0}));
    id += 1;
...
fn table_merge_split() {
    ...
    let r = s.call(id, "split_cell", json!({"handle": &h, "slide": 0, "shape_idx": 0, "row": 0, "col": 0}));
    id += 1;
...
fn table_sizing_and_cell_text_style() {
    ...
    let r = s.call(id, "set_cell_text", json!({"handle": &h, "slide": 0, "shape_idx": 0, "row": 0, "col": 0, "text": "Hello"}));
    id += 1;
    assert_eq!(r["status"], "success", "set_cell_text failed: {r}");

    let r = s.call(id, "set_cell_style", json!({"handle": &h, "slide": 0, "shape_idx": 0, "row": 0, "col": 0, "fill": "#FFD700"}));
    id += 1;
